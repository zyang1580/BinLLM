...
from minigpt4.models.rec_base_models import MatrixFactorization, MF_linear,LightGCN, SASRec, Personlized_Prompt, random_mf, Soft_Prompt, RecEncoder_DIN, hashGNN
...
class Rec2Base(BaseModel):
    ...
    def init_rec_encoder(self,rec_model, config, precision):
        if rec_model == "MF":
            ...
            print("### rec_encoder:", "using hash encoder, projection layer will not be used")
            rec_model = hashGNN(config)
            rec_model.set_encode_mode(config.code_mode)
