...
class MatrixFactorization(nn.Module):
    ...
    def do_rec(self, user, cliked):
        ...
        matching = torch.matmul(u_emb, i_emb.T)
        matching[cliked[0],cliked[1]] = -torch.inf
        _, rec_i = torch.topk(matching, 20, dim=-1)
        return rec_i
...
class hashGNN(nn.Module):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        self.eval_done_computer = False
        self.encode_mode = 'binary'
        self.use_hash = True
       

    def set_encode_mode(self,mode='binary'):
    ...
    def computer(self):
        if self.training or not self.eval_done_computer: # training or evaluation at first step
            ...
            if self.use_hash:
                hash_u = self.binarize(u_z).int()
                hash_i = self.binarize(i_z).int()
                hash_u[hash_u<0] = 0
                hash_i[hash_i<0] = 0
                self.hash_u, self.hash_i = self.hash2str(hash_u,hash_i,mode=self.encode_mode)
                self.hash_u = np.array(self.hash_u)
                self.hash_i = np.array(self.hash_i)
                if self.training:
                    self.eval_done_computer = False
                else:                                # evaluation will not repeatly computed the embedding if no traning is involved
                    if not self.eval_done_computer:  
                        self.eval_done_computer  = True

    
    def hash2str(self, hash_u, hash_i, mode='binary'):
        def arr2b(arr):
        ...
        if mode == 'binary':
            ...
            def arr2ipv4(arr):
                arr = torch.chunk(arr,4*(self.emb_dim//32),dim=1)
